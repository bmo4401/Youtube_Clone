...
export const getVideoDetails = createAsyncThunk('yotubeApp/videoDetails', async (id: string) => {
   const {
      data: { items },
   } = await axios.get(`${YOU_API_URL}/videos?key=${API_KEY}&part=snippet,statistics&type=video&id=${id}`);
...
const parseData = async (item: {
   ...
   } = await axios.get(`${YOU_API_URL}/channels?part=snippet,statistics&id=${item.snippet.channelId}&key=${API_KEY}`);
