...
import { YOU_API_URL } from './constants';
...
export const parseData = async (items: any[]) => {
   try {
      ...
      const {
         ...
         `${YOU_API_URL}/channels?part=snippet,contentDetails&id=${channelIds.join(',')}&key=${API_KEY}`,
      );
      ...
      const {
         ...
         `${YOU_API_URL}/videos?part=contentDetails,statistics&id=${videoIds.join(',')}&key=${API_KEY}`,
      );
